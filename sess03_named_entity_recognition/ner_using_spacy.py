...
perform NER on two sample paragraphs, then displays results in multiple ways:
  ...
  3. Rich ASCII tables with colors (via the 'rich' library if available)
  4. spaCy's built-in displaCy HTML visualizer saved to file
...
#collections = _check_import("collections", "pip install collections")
#rich = _check_import("rich", "pip install rich")
...
def load_nlp_model(model_name:str = 'en_core_web_md'):
  ...
  spaCy models bundle a tokenizer, POS tagger, dependency parser, and NER
...
def print_entities_grouped(doc) -> None:
  ...
  for label in sorted(grouped.keys()):
    ...
    entities = ", ".join(sorted(set(grouped[label]))) # Remove duplicates (duplicate)
    print(f"\n {label} ({description})"
      f" {entities}")


def print_entities_rich_tables(doc) -> None:
  """
  ...
  for n, ent in enumerate(doc.ents, start=1):
    description = spacy.explain(ent.label_) or "-"
    color = label_colours.get(ent.label_, "white") # Fixed: Pass ent.label_ directly, not as a list
    table.add_row(
        str(n),
        f"[{color}]{ent.text}[/{color}]",
        f"[{color}]{ent.label_}[/{color}]",
        description,
...
def save_display_html(doc, output_path: str = "ner_output.html") -> None:
  """
...
def main() -> None:
  ...
  7. Save displaCy HTML visualization to disk.
  """
  ...
  print_entities_rich_tables(doc)

  # Step 6. Summary Statistics
  print_summary(doc)

  # Step 7. Save displaCy HTML
  save_display_html(doc, output_path="../.app/files/ner_displacy.html")
