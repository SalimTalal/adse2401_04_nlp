...
===================================================================================================================
This program demonstrates a creative writing assistant that trains a small neural language model on a text
corpus and uses it to generate short story continuations. It covers the following features.
...
class LanguageModel(nn.Module):
    def __init__(
        ...
        # The output layer produces one score (logit) per vocabulary word.
        self.output = nn.Linear(hidden_dim, vocab_size)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # Look up embeddings for every word in the context window.
...
# -----------------------------------------------------------------------------------
# 2. CreativeWriter class (main script/program class)
# -----------------------------------------------------------------------------------
class CreativeWriter:
    ...
    embed_dim : int
        Word embedding dimensionality (default 50).
    hidden_dim : int
    ...
    """
    # -----------------------------------------------------------------------------------
    # Hyper-parameters deliberately exposed as class-level constant for
    ...
    def __init__(self, corpus_path:str = "..files/stories.txt") -> None:
        ...
        os.makedirs(os.path.dirname(corpus_path), exist_ok=True) # Ensure folder exist
        ...
        self.index_to_word: dict[str, str] = {}
    ...
    def load_corpus(self) -> bool:
        if not self.corpus_path.exists():
            print(f"\nERROR: Corpus file not found: {self.corpus_path}"
                  f"\n          Please create the file and add some story text. ")
            return False
    ...
    def train_model(self) -> None:
        if self.model is None or self.X_train is None:
            print("ERROR: Model or training data is not initialised")
            return

        # Cross-entropy loss is the standard choice for classification tasks.
        # Prdedicting the next word is a classification problem over the vocabulary.
        ...
        for epoch in range(1, self.EPOCHS + 1):
            ...
            if epoch % 10 == 0 or epoch == 1:
                print(f"    Epoch {epoch:>3}/{self.EPOCHS} Loss: {loss.item():.4f}")
    ...
    def predict_next_word(
            ...
            temperature: float = 1.0        # Default value of 1.0
        ) -> str:
        ...
        # Left-pad if fewer than CONTEXT_SIZE words were supplied.
        ...
        # Sample from the probability distribution (not argmax) so the output has
        # natural variation rather than always choosing the same word.
        predicted_index = int(
            np.random.choice(len(probabilities), 1, p=probabilities)
        )
    ...
    # -----------------------------------------------------------------------------------
    def generate_text(self, prompt:str, num_words: int = 40, temperature: float = 1.0) -> str:
...
def main()-> None:
    try:
        ...
        # --- Step 5: Build training Sequences -------------------------------------
        print("[ Step 5 ] Creating triaining sequences...")
        assistant.create_triaining_sequences()
        ...
        print(f"\nFATAL ERROR: An unexpected error occured: "
