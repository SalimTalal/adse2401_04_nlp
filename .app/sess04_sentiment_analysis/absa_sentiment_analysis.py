...
class ABSAAnalyzer:
    def __init__(
        ...
        for aspect, keywords in self._taxonomy.items():
            ...
            self._patterns[aspect] = [
                re.compile(rf"\b{re.escape(km)}\b",re.IGNORECASE) for km in sorted_kw
            ]
    ...
    def analyze(self,text:str) -> SentenceResult:
        ...
        return result
    def analyze_batch(self,text:list[str]) -> list[SentenceResult]:

        return [self.analyze(t) for t in text]
    ...
    def _split_clauses(text: str) -> list[str]:
        ...
        # Secondary split: "and the/a/an _" - new noun phrase starting
        clauses: list[str] = []
        for part in parts:
            sub = re.split(
...
def print_summary_table(results: list[SentenceResult]) -> None:
    ...
    for n, result in enumerate(results, start=1):
        ...
        if result.aspects:
            parts =  []
            for r in result.aspects:
                syn = _SENTIMENT_SYMBOLS[r.sentiment]
                color = _SENTIMENT_COLORS[r.sentiment]
                parts.append(_colorize(f"{syn} {r.aspect}", color))
            print("     " + "    |    ".join(parts))
